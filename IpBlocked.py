...
def is_subnet(entry: str):
    try:
        ...
        logging.error(f"Invalid IP or subnet: {entry}")
        return False
...
class IpBlockedAPI:
    ...
    def _get_unchecked_ips(self, df):
        try:
            ...
            logging.error(f"Error fetching unchecked IPs: {e}")
            raise

    def _process_ip_or_subnet(self, df, ip_or_subnet):
        try:
            if is_subnet(ip_or_subnet):
                self._update_dataframe_with_subnet_info(df, ip_or_subnet)
            else:
                self._update_dataframe_with_abuse_info(df, ip_or_subnet)
        except Exception as e:
            logging.error(f"Error occurred while processing IP or subnet: {e}")
            raise
    ...
    def _update_dataframe_with_abuse_info(self, df, ip):
        try:
            ...
            logging.error(f"Error updating DataFrame with abuse info: {e}")
            raise

    def _update_dataframe_with_subnet_info(self, df, subnet):
        logging.info(f"Checking subnet {subnet}")
    ...
    def _prepare_ip_data(self, abuse_result):
        try:
            ...
            logging.error(f"Error preparing IP data: {e}")
            raise
    ...
    def _update_or_append_dataframe(self, df, ip, ip_data):
        try:
            ...
            logging.error(f"Error updating or appending DataFrame: {e}")
            raise
